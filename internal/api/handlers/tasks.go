...
// Update the SubmitTask function to include proof_needed
func (h *TaskHandler) SubmitTask(c *fiber.Ctx) error {
	...
	var flagColor, flagName, proofDescription string
...
// Update the SubmitTasksBatch function to include proof_needed
func (h *TaskHandler) SubmitTasksBatch(c *fiber.Ctx) error {
	...
	for i, taskSubmission := range request.Tasks {
		...
		if taskSubmission.ProofDescription != nil {
			proofDescription = *taskSubmission.ProofDescription
		}
...
// Update the UpdateTask function to handle proofNeeded
